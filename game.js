...
const difficulties = {
  easy: { label: '簡單', hp: .78, damage: .78, block: .8, strength: 0 },
  normal: { label: '普通', hp: 1, damage: 1, block: 1, strength: 0 },
  hard: { label: '困難', hp: 1.28, damage: 1.22, block: 1.2, strength: 6 }
};
...
function scaleIntent(move) {
  ...
  if (scaled.damage && scaled.burn) scaled.text = `攻擊 ${scaled.damage}＋灼燒`;
  else if (scaled.damage) scaled.text = `${scaled.name === '終末崩落' ? '致命攻擊' : scaled.damage >= 150 ? '強攻' : '攻擊'} ${scaled.damage}`;
  else scaled.text = `護盾 ${scaled.block}＋強化`;
  return scaled;
}

function chooseIntent() {
  if (round % 5 === 0) return scaleIntent(enemyMoves[4]);
  if (enemy.hp / enemy.maxHp < .45 && round % 3 === 0) return scaleIntent(enemyMoves[1]);
  const pool = round < 3 ? [enemyMoves[0], enemyMoves[2], enemyMoves[3]] : enemyMoves.slice(0, 4);
  return scaleIntent(pool[Math.floor(Math.random() * pool.length)]);
}
...
function startBattle() {
  ...
  const difficulty = difficulties[selectedDifficulty];
  enemy.maxHp = enemy.hp = Math.round(enemyBase.hp * difficulty.hp);
  enemy.strength = difficulty.strength;
  ...
  $('#battleLog').innerHTML = '';
  addLog(`${player.name} 與獄骸進入第七觀測界域｜${difficulty.label}難度。`, 'system');
...
function buffMarkup(unit) {
  const buffs = [];
  if (unit.block) buffs.push({ icon: '⬡', name: '護盾', value: Math.ceil(unit.block), type: 'positive', tip: '優先吸收下一次受到的傷害' });
  if (unit.strength) buffs.push({ icon: '▲', name: '攻擊', value: `+${unit.strength}`, type: 'positive', tip: '提高每次攻擊造成的傷害' });
  if (unit.thorns) buffs.push({ icon: '✤', name: '反響', value: unit.thorns, type: 'positive', tip: '受到攻擊後對敵人造成反傷' });
  if (unit.burn) buffs.push({ icon: '♨', name: '灼燒', value: unit.burn, type: 'negative', tip: '回合結束時受到持續傷害' });
  if (unit.vulnerable) buffs.push({ icon: '◇', name: '破綻', value: unit.vulnerable, type: 'negative', tip: '受到的傷害提高 22%' });
  if (unit.weak) buffs.push({ icon: '▽', name: '弱化', value: unit.weak, type: 'negative', tip: '下一次攻擊傷害降低' });
...
$('#toSupportBtn').onclick = () => { selectedSupport = null; $('#selectedSupportName').textContent = '未選擇'; renderSupports(); renderDifficulty(); showScreen('supportScreen'); };
